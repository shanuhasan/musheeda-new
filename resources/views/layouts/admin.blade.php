    ...
    <div x-data="{ sidebarOpen: false }" class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out md:static md:translate-x-0 flex flex-col">
            ...
            <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
                ...
                @can('manage_pages')
                <a href="{{ route('admin.pages.index') }}" class="flex items-center px-4 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('admin.pages.*') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-gray-100' }} transition-colors">
