...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <a href="{{ route('admin.redirects.create') }}" class="inline-flex items-center justify-center gap-2.5 rounded-md bg-brand-500 px-4 py-2 text-center font-medium text-white hover:bg-brand-600 transition-colors">
        <span>
        ...
        <table class="w-full table-auto">
            <thead>
                <tr class="bg-slate-50 text-left dark:bg-slate-800">
                    <th class="min-w-[220px] px-4 py-4 font-medium text-slate-800 dark:text-white xl:pl-5">Old URL</th>
                    <th class="min-w-[220px] px-4 py-4 font-medium text-slate-800 dark:text-white">New URL</th>
                    <th class="min-w-[120px] px-4 py-4 font-medium text-slate-800 dark:text-white">Type</th>
                    <th class="px-4 py-4 font-medium text-slate-800 dark:text-white">Status</th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="border-b border-slate-200 px-4 py-5 pl-5 dark:border-slate-800">
                        <p class="text-slate-800 dark:text-white truncate max-w-xs" title="{{ $redirect->old_url }}">
                            /{{ $redirect->old_url }}
                        </p>
                    </td>
                    <td class="border-b border-slate-200 px-4 py-5 dark:border-slate-800">
                        @if($redirect->status_code == 410)
                            <span class="text-slate-400 italic">N/A (Gone)</span>
                        @else
                            <p class="text-slate-800 dark:text-white truncate max-w-xs" title="{{ $redirect->new_url }}">
                                /{{ $redirect->new_url }}
                            </p>
                        @endif
                    </td>
                    <td class="border-b border-slate-200 px-4 py-5 dark:border-slate-800">
                        <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-0.5 text-sm font-medium text-slate-800 dark:bg-slate-800 dark:text-slate-300">
                    ...
                    <td class="border-b border-slate-200 px-4 py-5 pr-5 text-right dark:border-slate-800">
                        <div class="flex items-center justify-end space-x-3">
                            <a href="{{ route('admin.redirects.edit', $redirect) }}" class="text-brand-500 hover:text-brand-600">
                            ...
                            <form action="{{ route('admin.redirects.destroy', $redirect) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this redirect?');">
                                @csrf
                                @method('DELETE')
