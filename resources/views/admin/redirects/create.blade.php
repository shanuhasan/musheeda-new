...
<form action="{{ route('admin.redirects.store') }}" method="POST" class="max-w-2xl">
    @csrf
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-white/[0.03] md:p-6 space-y-6">
        ...
        <div>
            ...
            <div class="flex items-center">
                ...
                <input type="text" name="old_url" value="{{ old('old_url') }}" required placeholder="old-page-path" class="w-full rounded-r-lg border border-slate-300 bg-transparent px-5 py-3 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-slate-700 dark:bg-slate-900 transition-all">
            </div>
        ...
        <div x-data="{ status: '{{ old('status_code', 301) }}' }">
            <div class="mb-6">
            ...
            <div x-show="status !== '410'">
                ...
                <div class="flex items-center">
                    <span class="rounded-l-lg border border-r-0 border-slate-300 bg-slate-50 px-3 py-3 text-slate-500 dark:border-slate-700 dark:bg-slate-800">/</span>
                    <input type="text" name="new_url" value="{{ old('new_url') }}" :required="status !== '410'" placeholder="new-page-path" class="w-full rounded-r-lg border border-slate-300 bg-transparent px-5 py-3 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-slate-700 dark:bg-slate-900 transition-all">
                </div>
        ...
        <div>
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-brand-500 rounded border-slate-300 focus:ring-brand-500 dark:border-slate-600 dark:bg-slate-700">
