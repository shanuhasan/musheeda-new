...
<form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-white/[0.03] md:p-6">
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <select name="author_id" id="author_id" class="w-full rounded-lg border border-slate-300 bg-transparent px-5 py-3 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-slate-700 dark:bg-slate-900 dark:focus:border-brand-500 transition-all">
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('author_id', $post->author_id ?? auth()->id()) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex items-center gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                    <input type="checkbox" name="show_toc" id="show_toc" value="1" {{ old('show_toc', $post->show_toc) ? 'checked' : '' }} class="h-5 w-5 rounded border-slate-300 text-brand-500 focus:ring-brand-500 dark:border-slate-700 dark:bg-slate-900">
        ...
        <!-- FAQs -->
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-white/[0.03] md:p-6" x-data="{ faqs: {{ json_encode(old('faqs', $post->faqs ?: [])) }} }">
        ...
        <!-- SEO -->
        @include('admin.partials.seo-form', ['model' => $post])
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-white/[0.03] md:p-6">
            ...
            <div class="max-h-48 overflow-y-auto space-y-2">
                @foreach($posts as $p)
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="related_posts[]" id="related_{{ $p->id }}" value="{{ $p->id }}" {{ in_array($p->id, old('related_posts', $post->relatedPosts->pluck('id')->toArray())) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-brand-500 focus:ring-brand-500">
