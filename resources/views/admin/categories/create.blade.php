...
@section('content')
<div class="mb-6">
    <h2 class="text-title-md2 font-bold text-slate-800 dark:text-white/90">Create Category</h2>
</div>
...
<form action="{{ route('admin.categories.store') }}" method="POST" class="max-w-2xl">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-white/[0.03] md:p-6 space-y-5">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" class="w-full rounded-lg border border-slate-300 bg-transparent px-5 py-3 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-slate-700 dark:bg-slate-900 transition-all">{{ old('description') }}</textarea>
